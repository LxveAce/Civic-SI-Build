...
```
[Cylinder Head] --> [Stock Cast Iron Exhaust Manifold] --> [Catalytic Converter] --> [Front Pipe / B-Pipe] --> [Resonator] --> [Rear Muffler] --> [Tailpipe/Tip]
```

| Component | Description | Connection Type |
|-----------|-------------|-----------------|
| **Exhaust Manifold** | Cast iron 4-2-1 log-style manifold, integrated heat shield | Bolted to cylinder head with M10x1.25 studs/nuts (8 total) |
| **Catalytic Converter** | Separate unit from manifold, bolted via flange | 2x 12mm spring bolts connect manifold to cat inlet flange |
| **Front Pipe (B-Pipe)** | Connects cat outlet to mid-section | Donut gasket + 2-bolt flange at cat end; welded to resonator at rear |
| **Resonator** | Small-canister resonator (NOT a full muffler) | Welded to front pipe; connects to rear muffler via donut gasket + 2-bolt flange |
| **Rear Muffler** | Large main muffler, oval canister | Bolted flange at inlet; welded to tailpipe at outlet |
| **Tailpipe** | Single exit, passenger side | Welded to rear muffler |
...
- Both are inline (series, not parallel)

### Pipe Diameters

| Section | OD (Outer Diameter) | Notes |
|---------|---------------------|-------|
| Manifold primaries | ~1.5" (38mm) | Very restrictive cast iron log |
| Manifold collector | ~1.75" (44mm) | Transitions to cat inlet |
| Catalytic converter | ~2.0" (51mm) | Internal substrate restriction |
| Front pipe / B-pipe | ~2.0" (51mm) | Some sources report 2.0-2.1" |
...
3. **Catalytic converter outlet to front pipe (B-pipe):** 2-bolt flange with donut gasket
4. **Resonator to rear muffler:** 2-bolt flange with donut gasket
5. Everything else is **welded** (front pipe to resonator is welded, rear muffler to tailpipe is welded)
...
| Brand | Part Number | Design | Material | Primaries | Collector | Price | Est. Gain | Notes |
|-------|-------------|--------|----------|-----------|-----------|-------|-----------|-------|
| **Skunk2 Alpha** | 412-05-1930 | 4-2-1 Tri-Y, stepped | T304 Stainless | Stepped (small, exact size undisclosed) | 2.5" | $570 | 8-15 WHP | Gold standard. Best all-around. |
| **PLM V2** | PLM-HFG1-V2-K24-HEADER | 4-2-1 Tri-Y, stepped | 18ga Stainless | 44mm->50mm stepped | 2.5" | $375 | 10-15 WHP | Best value. One-piece V2 design. |
| **PLM V2 + Cat** | PLM-HFG1-V2-K24-HEADER-CAT | 4-2-1 Tri-Y + 200-cell HFC | 18ga Stainless | 44mm->50mm stepped | 2.5" | $575 | 10-15 WHP | Includes bolt-on high-flow cat. |
...
- 44mm->50mm->2.5" stepped design is well-engineered
- Works with both K20 and K24 engines (future-proof if K24 swap is ever considered)
...
- 18-gauge tubing is thinner than some competitors
...
## 3. Front Pipe / Mid-Pipe Options

### What Connects the Header to the Rest of the Exhaust?

With the stock manifold, the catalytic converter bolts directly to the manifold via 2x 12mm spring bolts. The cat outlet then connects to the front pipe (B-pipe) via a 2-bolt flange with donut gasket.
...
### Aftermarket Front Pipe / Mid-Pipe Options
...
|--------|-------------|------|------|
| **Keep Stock Resonator** | Leave it as-is | Zero work, maintains quiet daily mode | Slightly restrictive at 2.0" |
...
| Product | Cell Count | Pipe Size | Price | Notes |
|---------|-----------|-----------|-------|-------|
| **Berk Technology BT1101-HFC-MET** | Metallic core (cell count not published) | 3" body, 70mm outlet | $350 | Best standalone option. Made in USA. J-bend O2 bung to prevent CEL. |
| **PLM 200-Cell (with header combo)** | 200-cell metallic | 2.5" | Included in $575 header+cat combo | Best value when buying with PLM header |
| **1320 Performance Catted Header** | 200-cell | 2.5" | ~$400-450 | Includes both header and cat |
...
[Cylinder Head]
     |
[Aftermarket Header] (4-2-1, 2.5" collector)
     |
[High-Flow Cat] (200-cell, 2.5" or adapter to 2.5")
     |  <-- 2.5" pipe
[QTP QTEC25 Cutout] (2.5" electric valve)
     |                    \
     |                  [Dump Pipe] (short, angled rearward, exits under car)
     |
[Resonator] (Vibrant Ultra Quiet 2.5" if upgrading pipe size, or stock if keeping 2.0")
     |
[Adapter: 2.5" to stock size] (if needed)
     |
[Stock Rear Muffler]
     |
[Stock Tailpipe]
...
- 2.5" Vibrant Ultra Quiet resonator (replaces stock resonator)
- 2.5" to stock adapter at rear muffler inlet flange

**Pros:** Consistent flow diameter, no bottlenecks, maximum performance
**Cons:** More fabrication work, higher cost, requires muffler shop

#### Option B: Header + Cat with Stock Piping (Budget)

Use the PLM V2 + Cat combo, which transitions down to mate with the stock exhaust at the existing flange point. Keep all stock piping from the cat outlet back.

**Pros:** Minimal fabrication, lower cost, bolt-on simplicity
**Cons:** Stock 2.0" piping becomes the bottleneck after the 2.5" header collector, leaves some power on the table (estimated 3-5 WHP lost vs full 2.5")
...
## 6. Pipe Diameter Analysis

### The Physics

Exhaust pipe diameter is a balance between two opposing forces:
- **Too small:** Creates backpressure, restricts flow at high RPM, limits peak power
- **Too large:** Reduces exhaust gas velocity, hurts low-end torque and throttle response (scavenging effect weakened)

For naturally aspirated engines, maintaining exhaust gas velocity of 200-250 feet per second (FPS) is the target for optimal scavenging.

### K20Z3 Specific Analysis

| Engine Output | Optimal Pipe Diameter | Notes |
|---------------|----------------------|-------|
| Stock (~197 HP / ~170 WHP) | 2.0-2.25" | Stock 2.0" is fine at stock power |
| Bolt-on (~210-225 WHP) | 2.25-2.5" | 2.5" is the sweet spot for full bolt-on |
...
The K20Z3 is rated at 197 HP (crank) / 139 lb-ft (crank). At the wheels, stock cars typically dyno:
- **Dynojet:** 175-185 WHP / 130-138 WTQ
- **Mustang Dyno:** 165-175 WHP / 125-132 WTQ (reads lower than Dynojet)

### Gains by Component (Approximate, with Hondata Tune)

| Modification | WHP Gain | WTQ Gain | Notes |
|--------------|----------|----------|-------|
| **Hondata FlashPro tune alone** (stock hardware) | 5-10 | 3-8 | VTEC point optimization, ignition timing, fuel map |
| **Cold air intake + tune** | 8-15 | 5-10 | Intake + tune is greater than sum of parts |
| **Header + tune** (stock exhaust) | 10-18 | 8-15 | Biggest single bolt-on gain |
| **High-flow cat** | 3-7 | 5-10 | Modest but noticeable, especially torque |
...
|-------|------|-----|-----|--------|
| Skunk2 Alpha + 3" exhaust + intake + tune | Dynopack | 226 | 160 | k20a.org |
| PLM header + intake + tune | Dynojet | 234 | 165 | 8thcivic.com |
| K-Tuned header + intake + tune | Dynojet | 194 | 142 | 8thcivic.com |
...
| Connection Point | Risk Level | Notes |
|------------------|-----------|-------|
| **Manifold studs (head side)** | HIGH | Most likely to break. PB Blaster for days beforehand. |
| **Manifold-to-cat spring bolts** | MEDIUM | Spring bolts corrode. Have replacements ready. |
| **Cat-to-front-pipe flange bolts** | MEDIUM | Donut gasket will be crushed/fused. Replace gasket. |
| **O2 sensor** | LOW-MEDIUM | May be seized in the bung. Use O2 sensor socket + penetrant. |
...
|---------------|----------------|-----|
| **Low RPM** | Deeper, more rumble | Hollow, slightly droning |
| **Mid RPM** | Full-bodied, aggressive | Moderate, building |
| **High RPM** | Raspy, metallic, "Honda" | Screaming, high-pitched wail |
| **Overall character** | Balanced tone across the band | Crescendo that builds to a peak |
| **Best described as** | "Angry Honda" | "Race car" |
...
With the cutout open, exhaust dumps before the resonator and muffler:

| Header Type | Sound Character |
|-------------|----------------|
...
| Brand | Status |
|-------|--------|
| **Toda Racing** | Does not make a header for the 8th gen K20Z3 SI |
| **Weapon R** | Makes an R18 header (base Civic), not a K20Z3 header |
| **DC Sports** | Discontinued their K20Z3 header; very difficult to find |
...
3. Retune both Hondata calibrations (strongly recommended on dyno)
4. Verify AFR in both valve positions with the new header
5. Re-check torque on all exhaust connections after 100 miles of heat cycling
...
*Sources: 8thcivic.com, k20a.org, Skunk2, PLM/Private Label Mfg, K-Tuned, HARDmotion, Berk Technology, Vibrant Performance, QTP, Hybrid Racing, various community dyno threads*
